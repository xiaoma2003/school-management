    ...
    <div class="content">
        ...
        <form action="<%= ctx %>/equipment/save" method="post" onsubmit="return validateForm()">
            ...
            <div class="form-group">
                ...
                <input type="text" name="equipmentNo" id="equipmentNo" value="${equipment.equipmentNo}" required placeholder="请输入设备编号">
                <div class="error" id="equipmentNoError"></div>
            </div>
            <div class="form-group">
                <label>设备名称<span class="required">*</span></label>
                <input type="text" name="equipmentName" id="equipmentName" value="${equipment.equipmentName}" required placeholder="请输入设备名称">
            </div>
            <div class="form-group">
                <label>所属部门</label>
                <select name="deptId" id="deptSelect">
                    ...
                    <c:forEach items="${depts}" var="dept">
                        <option value="${dept.deptId}" ${equipment.deptId == dept.deptId ? 'selected' : ''}>${dept.deptName}</option>
                    </c:forEach>
                </select>
            </div>
            <div class="form-group">
                ...
                <select name="classId" id="classSelect">
                    ...
                    <c:forEach items="${classes}" var="cls">
                        <option value="${cls.classId}" ${equipment.classId == cls.classId ? 'selected' : ''}>${cls.gradeName} - ${cls.className}</option>
                    </c:forEach>
                </select>
            </div>
            <div class="form-group">
                <label>状态</label>
                <select name="status">
                    <option value="1" ${equipment.status == 1 ? 'selected' : ''}>正常</option>
                    <option value="2" ${equipment.status == 2 ? 'selected' : ''}>故障</option>
                </select>
            </div>
            <div class="form-group">
                <label>备注</label>
                <textarea name="remark" placeholder="请输入备注信息">${equipment.remark}</textarea>
            </div>
