...
<body>
    ...
    <div class="content">
        ...
        <h2>编辑学生</h2>
        <form action="<%= ctx %>/school/student/save" method="post">
            ...
            <div class="form-group">
                <label>出生日期</label>
                <input type="date" name="birthday" value="${student.birthday != null ? student.birthday.toString().substring(0, 10) : ''}">
            </div>
            <div class="form-group">
                <label>班级</label>
                <select name="classId" required>
                    ...
                    <c:forEach items="${classes}" var="clazz">
                        <option value="${clazz.classId}" ${student.classId == clazz.classId ? 'selected' : ''}>${clazz.gradeName} - ${clazz.className}</option>
                    </c:forEach>
                </select>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label>状态</label>
                <select name="status">
                    <option value="1" ${student.status == 1 ? 'selected' : ''}>正常</option>
                    <option value="0" ${student.status == 0 ? 'selected' : ''}>禁用</option>
                </select>
            </div>
