    ...
    <div class="content">
        ...
        <form action="<%= ctx %>/school/class/save" method="post">
            ...
            <div class="form-group">
                <label>班级名称</label>
                <input type="text" name="className" value="${clazz.className}" required>
            </div>
            <div class="form-group">
                <label>所属年级</label>
                <select name="gradeId" required>
                    ...
                    <c:forEach items="${grades}" var="grade">
                        <option value="${grade.gradeId}" ${clazz.gradeId == grade.gradeId ? 'selected' : ''}>${grade.gradeName}</option>
                    </c:forEach>
                </select>
            </div>
            <div class="form-group">
                <label>班主任</label>
                <input type="text" name="teacherName" value="${clazz.teacherName}">
            </div>
            <div class="form-group">
                <label>排序</label>
                <input type="number" name="sortOrder" value="${clazz.sortOrder}">
            </div>
            <div class="form-group">
                <label>状态</label>
                <select name="status">
                    <option value="1" ${clazz.status == 1 ? 'selected' : ''}>启用</option>
                    <option value="0" ${clazz.status == 0 ? 'selected' : ''}>禁用</option>
                </select>
            </div>
